...
| `scripts/25_eval_llm_systems.py` | **ROADMAP v2 Day 4.** LLM-family evaluation on the 296-item GT. Targets Gemini 2.5 Flash-Lite (Google AI Studio REST, requires `GEMINI_API_KEY`), Llama-3.1-8B-Instant (Groq REST, OpenAI-compatible), and AceGPT-7B-chat int4 GGUF (local via `llama-cpp-python`). Zero-shot and 3-shot prompts; strict JSON output (`is_lebanese`, `confidence`); P(Lebanese) derived from confidence. Per-system pause/resume + partial checkpoints every 20 items via `data/benchmark_predictions/<name>.partial.json`. Writes FINDINGS Section 20. |
