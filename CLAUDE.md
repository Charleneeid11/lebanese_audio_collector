...
| `scripts/25_eval_llm_systems.py` | **ROADMAP v2 Day 4.** LLM-based zero-shot and 3-shot dialect classification using Gemini REST API, Groq REST API, and local AceGPT (llama-cpp-python GGUF). Requires `GROQ_API_KEY` in `.env` for Groq path. Gemini free tier = 20 RPD (impractical for 296 items). AceGPT requires GGUF at `models/AceGPT-7B-chat.Q4_K_M.gguf`. **Status: pending — venv rebuilt 2026-06-29; Groq API key needed from user.** |
| `scripts/26_per_platform_breakdown.py` | **ROADMAP v2 Day 5.** Per-platform (podcast_rss, youtube, adi17, fleurs) ROC-AUC and macro F1 for all 12 completed systems. Also computes code-switching density per GT item from screening transcripts. Outputs `data/benchmark_per_platform.csv`, `data/code_switching_features.csv`. Appends FINDINGS Section 22 (idempotent). **Added 2026-06-29.** |
| `scripts/27_generate_figures.py` | **ROADMAP v2 Day 7.** Generates all thesis figures: ROC curve grid (all 12 systems, one panel), confusion matrix small-multiples (6 selected systems), per-platform ROC-AUC bar chart (podcast_rss vs youtube). Outputs `paper/figures/roc_curves.pdf/.png`, `paper/figures/confusion_matrices.pdf/.png`, `paper/figures/per_platform_bar.pdf/.png`. Requires `data/benchmark_per_platform.csv` (run script 26 first). **Added 2026-06-29.** |
| `scripts/28_aldi_correlation.py` | **ROADMAP v2 Day 6.** Scores each GT item's screening transcript with `AMR-KELEG/ALDi` (BERT-based Arabic dialectness regressor [0,1]). Computes Spearman r between ALDi score and per-system prediction error for all 14 systems. Outputs `data/aldi_scores.csv`, `data/aldi_correlation.json`. Appends FINDINGS Section 24. Requires `torch` + `transformers` in venv. **Added 2026-07-04.** |
| `scripts/29_same_source_control.py` | **ROADMAP v2 Day 6.** Same-source control (P4): trains MLP on 1024-d XLS-R embeddings using only podcast_rss items (same domain as 85% of GT). Disambiguates "domain shortcut" from "weak acoustic signal" explanations. Adds `v2_same_source` row to benchmark CSV. Outputs `models/dialect_classifier_v2_same_source.joblib`, `data/benchmark_predictions/v2_same_source.json`. Appends FINDINGS Section 25. **2026-07-04: podcast_rss ROC-AUC 0.7392 vs V2-frozen 0.7906 — same-source training did NOT recover performance, confirming XLS-R lacks dialect signal regardless of training domain.** |
| `scripts/30_failure_cases.py` | **ROADMAP v2 Day 7 (remainder).** Selects top-10 most-confidently-wrong items per system (highest abs(prob − label)). Joins with screening transcripts. Outputs `data/failure_cases.csv` (system, rank, item_id, platform, true_label, pred_prob, error, failure_type, transcript_text, audio_path). 15 systems × 10 = 150 rows. **Added 2026-07-04.** |
